...
<div class="relative min-h-screen bg-[#1B3A6B] flex items-center justify-center px-6 py-6">
  ...
  <div class="w-full max-w-[1800px] min-h-[calc(100vh-3rem)] border-[0px] border-[#1c2f5e] box-border p-10 flex flex-col">
    ...
    <!-- Back button -->
    <div class="mb-6">
      <a href="{{ route('leave-requests.index') }}" class="inline-flex items-center gap-2 bg-[#0061FF20] hover:bg-[#0061FF40] text-white text-[13px] font-semibold tracking-wide px-5 py-2.5 rounded-full border border-[#2a3f72] transition-colors">
    ...
    <div class="flex-1 bg-[#132B52] rounded-2xl border border-[#22335f] shadow-2xl px-14 py-10 flex flex-col gap-8 overflow-hidden">
      ...
      <div>
        <h1 class="text-white text-2xl font-400 tracking-wide">
          LEAVE REQUEST ID: <span class="text-[#5b8def] font-bold">#{{ $leave->reference_id }}</span>
        </h1>
      </div>
     ...
     <!-- PROFILE SECTION -->
      ...
      <div class="rounded-xl border border-[#22335f] overflow-hidden">
        ...
        <div class="grid grid-cols-4 divide-x divide-[#22335f] bg-[#0B1E3D]">
          <div class="px-8 py-6">
            ...
            <p class="text-white text-[15px]">{{ ucfirst($leave->type) }}</p>
          </div>
          <div class="px-8 py-6">
            ...
            <p class="text-slate-400 text-[12px] mt-1 ml-5">({{ \Carbon\Carbon::parse($leave->from_date)->format('l') }})</p>
          </div>
          <div class="px-8 py-6">
            <p class="text-white text-[13px] font-400 mb-2">To</p>
            <div class="flex items-center gap-2">
              <p class="text-white text-[15px] font-semibold">{{ \Carbon\Carbon::parse($leave->to_date)->format('F d, Y') }}</p>
            </div>
            <p class="text-slate-400 text-[12px] mt-1 ml-5">({{ \Carbon\Carbon::parse($leave->to_date)->format('l') }})</p>
          </div>
          <div class="px-8 py-6">
            <p class="text-white text-[13px] font-400 mb-2">Total</p>
            <p class="text-[#5b8def] text-2xl font-bold">{{ \Carbon\Carbon::parse($leave->from_date)->diffInDays($leave->to_date) + 1 }} Days</p>
          </div>
        </div>
        <div class="bg-[#0B1E3D] px-8 py-6 border-t border-white/10">
          <p class="text-white text-[13px] font-400 mb-2">Employee Remarks</p>
          <p class="text-slate-200 text-[15px]">{{ $leave->reason ?? 'No remarks provided.' }}</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-8">
        ...
        <div class="rounded-xl border border-[#22335f] overflow-hidden flex flex-col">
          ...
          <div class="bg-[#0B1E3D] px-6 py-5 flex-1 flex items-center">
            @php
              $s = match($leave->status) {
                'approved' => ['label' => 'APPROVED', 'color' => 'text-green-400', 'note' => $leave->status_note ?? 'Your request has been approved.'],
                'rejected' => ['label' => 'REJECTED', 'color' => 'text-red-400', 'note' => $leave->status_note ?? 'Your request has been rejected.'],
      ...
      <!-- HR REMARKS -->
      <form method="POST" action="{{ route('leave-requests.review', $leave->id) }}">
        @csrf
        <div class="rounded-xl border border-[#22335f] overflow-hidden">
          <div class="flex items-center gap-2 bg-[#132a58] px-6 py-3.5 border-b border-[#22335f]">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
